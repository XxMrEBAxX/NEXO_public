...
namespace BirdCase
{
    public class ActiveAsciiArt : MonoBehaviour
    {
        [SerializeField]
        ...
        private async UniTaskVoid Fade(Vector3 endSize)
        {
            float duration = fadeDuration * 0.5f;
            float elapsedTime = 0;
            Vector3 startSize = panelImage.transform.localScale;
            Vector3 resultSize = new Vector3(endSize.x, startSize.y, 1);
